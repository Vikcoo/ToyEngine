...
static GLenum ShaderStageToGL(RHIShaderStage stage)
{
    switch (stage)
    {
        ...
        default:
            TE_LOG_ERROR("[OpenGLRHI] Unsupported shader stage");
    ...
    : m_Stage(desc.stage)
{
    ...
    {
        TE_LOG_ERROR("[OpenGLRHI] Failed to read shader file: {}", desc.filePath);
        return;
    }
    ...
    if (m_ShaderID == 0)
    {
        TE_LOG_ERROR("[OpenGLRHI] Failed to create shader object");
        return;
    }
    ...
    {
        TE_LOG_DEBUG("[OpenGLRHI] Shader '{}' compiled successfully (ID={})", desc.debugName, m_ShaderID);
    }
}
...
bool OpenGLShader::ReadShaderFile(const std::string& filePath, std::string& outSource)
{
    ...
    if (!file.is_open())
    {
        TE_LOG_ERROR("[OpenGLRHI] Cannot open shader file: {}", filePath);
        return false;
    }
    ...
    if (outSource.empty())
    {
        TE_LOG_WARN("[OpenGLRHI] Shader file is empty: {}", filePath);
        return false;
    }
...
bool OpenGLShader::Compile(const std::string& source)
{
    ...
    if (!success)
    {
        ...
        TE_LOG_ERROR("[OpenGLRHI] Shader compilation failed:\n{}", infoLog);
        return false;
    }
