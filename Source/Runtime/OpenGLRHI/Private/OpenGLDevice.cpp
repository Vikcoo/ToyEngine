...
{
    TE_LOG_INFO("[OpenGLRHI] OpenGL Device initialized");
    TE_LOG_INFO("[OpenGLRHI] GL_VENDOR: {}", reinterpret_cast<const char*>(glGetString(GL_VENDOR)));
    TE_LOG_INFO("[OpenGLRHI] GL_RENDERER: {}", reinterpret_cast<const char*>(glGetString(GL_RENDERER)));
    TE_LOG_INFO("[OpenGLRHI] GL_VERSION: {}", reinterpret_cast<const char*>(glGetString(GL_VERSION)));
}

OpenGLDevice::~OpenGLDevice()
{
    TE_LOG_INFO("[OpenGLRHI] OpenGL Device destroyed");
}
...
std::unique_ptr<RHIShader> OpenGLDevice::CreateShader(const RHIShaderDesc& desc)
{
    ...
    if (!shader->IsValid())
    {
        TE_LOG_ERROR("[OpenGLRHI] Failed to create shader: {}", desc.filePath);
        return nullptr;
    }
    return shader;
}
...
std::unique_ptr<RHIPipeline> OpenGLDevice::CreatePipeline(const RHIPipelineDesc& desc)
{
    ...
    if (!pipeline->IsValid())
    {
        TE_LOG_ERROR("[OpenGLRHI] Failed to create pipeline");
        return nullptr;
    }
    return pipeline;
}
