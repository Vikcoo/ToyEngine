    ...
    , m_VertexInput(desc.vertexInput)
{
    ...
    {
        TE_LOG_ERROR("[OpenGLRHI] Pipeline creation failed: invalid vertex shader");
        return;
    }

    if (!fragShader || !fragShader->IsValid())
    {
        TE_LOG_ERROR("[OpenGLRHI] Pipeline creation failed: invalid fragment shader");
        return;
    }
    ...
    {
        TE_LOG_DEBUG("[OpenGLRHI] Pipeline '{}' created: Program={}, VAO={}", desc.debugName, m_ProgramID, m_VAO);
    }
}
...
bool OpenGLPipeline::LinkProgram(GLuint vertShader, GLuint fragShader)
{
    ...
    if (!success)
    {
        ...
        TE_LOG_ERROR("[OpenGLRHI] Program link failed:\n{}", infoLog);
