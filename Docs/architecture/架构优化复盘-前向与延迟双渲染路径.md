...
1. ~~**渲染循环是单一 BasePass 扁平流程**。~~（已在 Stage 2 修正）[`FSceneRenderer::Render`](../../Source/Runtime/Renderer/Private/SceneRenderer.cpp) 现只调度当前 `IRenderPath`；默认 `FForwardRenderPath` 通过 `FMeshPassProcessor` 构建 BasePass 命令并保持旧 Forward 行为等价。Deferred 具体路径仍待 Stage 4 落地。
...
- `SetUniform*` 目前只用在 OpenGL 后端的小量核心变量（MVP、Model、BaseColorTex、Forward 光源数组），集中在 `FForwardRenderPath::SubmitDrawCommands` 一处，清理代价可估。
- 真正的迁移触发点是接入 Vulkan 后端——那时不得不全面走 BindGroup。本次改造为止留出清晰的迁移边界即可。

**具体边界**：

- `u_MVP` / `u_Model`：保持旧路径，在 `FForwardRenderPath` 内用 `SetUniformMatrix4`。
...
- 当前仍只有 `StaticMeshBasePass` 一条真实 Pipeline，`GBuffer` / `Lighting` 只是 key 维度预留。
- Shader 选择逻辑仍在 `FRenderResourceManager` 内部硬编码到 `model.vert` / `model.frag`，Stage 4 引入更多 Pass 时需要继续拆成按 key 选择 shader 描述。
...
- 当前只有 `FForwardRenderPath`，没有 `FDeferredRenderPath`。
...
（后续 Stage 4 ~ Stage 5 完成后依次向下追加记录，保持相同结构：为什么要改 / 设计取舍 / 落地文件 / 验证方式 / 收益 / 仍然保留的限制。）
