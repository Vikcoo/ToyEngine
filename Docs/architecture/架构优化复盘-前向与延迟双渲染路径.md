...
当前项目已切到 OpenGL 4.5 Core，`LightBlock` 可直接在 GLSL 中写 `binding = 0`。OpenGL 后端仍保留 `glUniformBlockBinding(program, blockIndex, 0)` 作为 Program 侧显式对齐，便于调试和避免状态漂移。
...
- `model.frag` 与 `deferred_lighting.frag` 改为读取 `layout(std140, binding = 0) uniform LightBlock`，并在 OpenGL Program 链接后继续显式调用 `glUniformBlockBinding(..., 0)` 对齐程序状态。
...
- `u_MVP`、`u_Model`、调试视图等参数仍在使用过渡期 name-based uniform 接口，资源绑定模型还没有彻底收口。
