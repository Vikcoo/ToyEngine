...
1. ~~**渲染循环是单一 BasePass 扁平流程**。~~（已在 Stage 4 修正）[`FSceneRenderer::Render`](../../Source/Runtime/Renderer/Private/SceneRenderer.cpp) 现只调度当前 `IRenderPath`；`FForwardRenderPath` 与 `FDeferredRenderPath` 均已落地，当前默认使用 Deferred 路径。运行时切换入口仍待 Stage 5。
...
- **Stage 4 — Deferred 路径**（已完成）：GBuffer Pass（MRT）+ 全屏三角形 Lighting Pass；解决 7.3 的 Y 翻转；补强 `OpenGLPipeline` 对空 `vertexInput.bindings` 的健壮性。当前默认路径临时切到 Deferred，运行时切换留到 Stage 5。
- **Stage 5 — 切换入口 + 文档收尾**：`Engine::SetRenderPath()`；Sandbox F1 / F2 切换；更新 `Docs/guides/渲染管线.md`、`Docs/architecture/运行时模块.md`、`Docs/architecture/单帧执行流.md`；在 `Docs/reference/临时架构问题清单.md` 里记录留坑（资源屏障接口、SPIR-V 管线、材质系统、光源 UBO 迁移）。
...
- ~~**没有在 Sandbox 引入 RT 使用示范**。~~ Stage 4 当前默认 `FDeferredRenderPath` 会创建 GBuffer RT 并执行 Lighting Pass。
...
- `FForwardRenderPath` 与 `FDeferredRenderPath` 均已落地；当前默认路径已临时切到 Deferred。
- `FMeshPassProcessor` 仍直接调用 Proxy 的 `GetMeshDrawCommands`，尚未引入显式 `FMeshBatch`。
- 默认路径仍在构造 `FSceneRenderer` 时固定，运行时切换入口留到 Stage 5。
...
本阶段新增 `FDeferredRenderPath`，当前默认路径临时切到 Deferred 以便端到端运行。运行时切换入口仍留给 Stage 5，避免本阶段同时改 Engine 输入和路径管理 API。

- GBuffer Pass 复用 `FMeshPassProcessor(BasePass)` 产出的 `FMeshDrawCommand`，但绑定 Deferred 自己的 GBuffer pipeline。
- GBuffer RT 使用两个 color attachment：`RGBA8_UNorm` Albedo、`RGBA8_UNorm` 编码 WorldNormal，再加 `D32_Float` Depth。
- Lighting Pass 使用全屏三角形和空 vertex input，采样 Albedo / Normal / Depth，使用逆 ViewProjection 从深度重建世界坐标。
...
- 当前默认路径直接切到 Deferred，但还没有 `Engine::SetRenderPath()` 或 Sandbox 快捷键切换；Stage 5 处理。
- GBuffer 只存 Albedo、WorldNormal、Depth，没有 roughness/metallic/emissive/shading model。
...
（后续 Stage 5 完成后继续追加记录，保持相同结构：为什么要改 / 设计取舍 / 落地文件 / 验证方式 / 收益 / 仍然保留的限制。）
