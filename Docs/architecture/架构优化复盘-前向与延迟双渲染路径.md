...
当前项目已切到 OpenGL 4.5 Core，`LightBlock` 直接依赖 GLSL 中的 `binding = 0` 声明，不再保留额外的 Program 侧冗余绑定逻辑。
...
- `u_DirectionalLight*` / `u_PointLight*`：Stage 3 暂时走 name-based uniform 数组；后续迁移到 `LightBlock` UBO。（已在后续补充阶段完成）
...
| 渲染线程 | 单线程帧包直传 | 场景变更已命令化；计划中：有界队列与独立 RenderThread |
| MSAA | 不支持 | 最小版不做 |
...
- `FForwardRenderPath` 每次提交 DrawCommand 时从 `FScene::GetLights()` 取光源，shader 内循环累加最多 4 个方向光和 8 个点光。

Stage 3 当时刻意没有连带做 `LightBlock` UBO，因为那时 RHI 还缺少清晰的 per-frame uniform buffer 更新路径。后续阶段补上 `RHIBuffer::UpdateData()` 之后，光源上传再迁移到 BindGroup/UBO。
...
- Stage 3 当时的限制已经在后续阶段通过 `LightBlock` UBO / BindGroup 补齐；这里保留记录用于追溯当时的设计取舍。
...
- Lighting Pass 当前已通过共享 `LightBlock` UBO / BindGroup 上传光源。
...
### Stage 5 补充修正 — LightBlock UBO 迁移（已完成，2026-05-21）

#### 为什么要改

前面的 Forward / Deferred 路径虽然都已经能工作，但光源数据仍通过 name-based uniform 数组逐条写入。这有两个明显问题：

- CPU 每次提交 draw/pass 都要重复拼 uniform 名称并逐项设置，开销和可维护性都不好。
- 这条路径本质上是 OpenGL 过渡接口，不适合作为 Vulkan / D3D12 长期资源绑定模型。

Stage 3 当时没有做这件事，不是因为不需要，而是因为 RHI 还没有可靠的 per-frame uniform buffer 更新能力。现在补齐 `RHIBuffer::UpdateData()` 后，这个迁移条件才真正成立。

#### 修正方式

- 在 `RHIBuffer` 抽象层增加 `UpdateData()`。
- OpenGL 后端用 `glBufferSubData` 实现 UBO 内容更新，并把 Uniform Buffer 默认 hint 调整为 `GL_DYNAMIC_DRAW`。
- `RendererLightUniforms` 从“直接写 name-based uniform”改成维护 `FLightUniformBindingState`：
  - 一个 `UniformBuffer`
  - 一个 `BindGroup`
- Forward / Deferred 两条路径都通过 `UpdateAndBindSceneLightUniforms()` 更新并绑定共享的 `LightBlock`。
- `model.frag` 与 `deferred_lighting.frag` 改为读取 `layout(std140, binding = 0) uniform LightBlock`，完全依赖 shader 中的 binding 声明与 BindGroup 绑定点对齐。

#### 结果

- 光源上传从逐项 name-based uniform 调用，迁移为一次 UBO 更新 + 一次 BindGroup 绑定。
- Forward / Deferred 两条路径现在共享同一套光源上传逻辑。
- `LightBlock` 的 shader 形态和 RHI 绑定模型更接近后续 Vulkan / D3D12 的实现方向。

#### 仍然保留的限制

- `LightBlock` 目前仍是单个全局块，没有进一步拆到更通用的 frame/view/pass 常量缓冲体系。

### Stage 5 补充修正 — ObjectBlock / DeferredPassBlock 迁移（已完成，2026-06-04）

#### 为什么要改

`LightBlock` 迁移完成后，主渲染路径里仍残留两类最明显的 OpenGL 过渡接口：

- 每个 draw 通过 `SetUniformMatrix4 / SetUniformMatrix3` 上传 `u_MVP / u_Model / u_NormalMatrix`
- Deferred Lighting Pass 通过 `SetUniformInt` 上传 RT 翻转标志和调试视图模式

这会让 Forward / Deferred 虽然已经用上了 UBO，但真正的主路径仍没有彻底脱离 name-based uniform。

#### 修正方式

- 新增 `RendererPassUniforms`，集中管理两类常量块：
  - `ObjectBlock`：`MVP / Model / NormalMatrix`
  - `DeferredPassBlock`：RT 采样翻转标志、调试视图模式
- `model.vert` / `gbuffer.vert` 改为读取 `layout(std140, binding = 1) uniform ObjectBlock`
- `deferred_lighting.frag` 改为读取 `layout(std140, binding = 1) uniform DeferredPassBlock`
- `ForwardRenderPath` / `DeferredRenderPath` 不再调用 `SetUniformMatrix* / SetUniformInt`
- `u_BaseColorTex` 与 GBuffer 采样器补上显式 `layout(binding = N)`，避免再通过 `SetUniformInt` 指定纹理槽位

#### 结果

- 主渲染路径中的矩阵上传和 Deferred 调试参数上传都已收口到 UBO / BindGroup。
- Forward / Deferred 正常绘制不再依赖 `SetUniformMatrix* / SetUniformInt`。
- 过渡期旧接口的实际使用面进一步缩小到纹理绑定与少量示例 shader。

### Stage 5 补充修正 — 纹理采样迁移到 BindGroup（已完成，2026-06-04）

#### 为什么要改

前一阶段虽然已经把 `SetUniformMatrix* / SetUniformInt` 从主路径清掉，但 Forward 的 BaseColor 和 Deferred 的 GBuffer 仍通过 `BindTexture2D(slot, ...)` 逐条绑定。

这意味着：

- 主路径仍保留一半“按资源组绑定”、一半“按槽位逐条绑定”的混合模式
- `BaseColor = 2`、`GBufferAlbedo = 2`、`Normal = 3` 这类布局信息散落在提交代码里
- CPU 侧虽然不再依赖 name-based uniform，但仍然在手写 texture unit 约定

#### 修正方式

- 新增 `RendererBindingSlots`，集中声明当前主路径的 binding 槽位常量。（后续已拆分为 `RendererBindGroups` / `RendererBindings`）
- 新增 `RendererTextureBindings`，集中管理两类纹理 BindGroup：
  - `BaseColorTexture_BindGroup`
  - `GBufferTextures_BindGroup`
- `ForwardRenderPath` 不再直接调用 `BindTexture2D(2, ...)`，改为通过 `UpdateAndBindBaseColorTexture()` 绑定 BaseColor 纹理组
- `DeferredRenderPath` 不再逐条绑定 GBuffer 附件，改为通过 `UpdateAndBindGBufferTextures()` 一次性绑定 `Albedo / Normal / WorldPosition / Depth`

#### 结果

- Forward / Deferred 主路径现在不再调用 `BindTexture2D`
- 主路径的 UBO、光照数据、Deferred 调试参数、主纹理采样都统一走 BindGroup
- CPU 侧 binding 数字从散落常量收口到 `RendererBindingSlots`。（后续已拆分为 group index 与 binding slot 两类常量）

#### 仍然保留的限制

- shader 里的 `layout(binding = N)` 与 C++ 里的资源布局声明仍是两份声明，还没有上升到单一真相源。
- 若后续继续演进，应建立 shader 反射或代码生成机制，让资源布局自动从 shader/中间表示导出到运行时代码。

### Stage 6 补充修正 — 移除旧版 RHI 资源绑定接口（已完成，2026-06-11）

#### 为什么要改

Stage 4 / Stage 5 已经让 Forward / Deferred 主路径不再调用 `SetUniform*` 和 `BindTexture2D`，但 `RHICommandBuffer` 公共接口与 `OpenGLCommandBuffer` 实现仍保留这些 OpenGL 过渡接口。它们会继续污染 RHI 抽象层，并迫使 Vulkan / D3D12 后端未来面对没有自然语义的接口。

#### 修正方式

- 从 `RHICommandBuffer` 删除 `SetUniformMatrix4`、`SetUniformMatrix3`、`SetUniformFloat`、`SetUniformVec3`、`SetUniformInt`。
- 从 `RHICommandBuffer` 删除旧版 `BindTexture2D`。
- 从 `OpenGLCommandBuffer` 删除对应实现。
- 资源绑定统一保留 `SetBindGroup()` 路径。

#### 结果

- RHI 公共命令接口不再暴露 OpenGL name-based uniform 或旧 texture unit 绑定模型。
- OpenGL 后端和未来 Vulkan / D3D12 后端共享同一套 `Buffer / Texture / Sampler + BindGroup + Layout` 资源进入 shader 的路径。

#### 仍然保留的限制

- shader 里的 `layout(binding = N)` 与 C++ 里的 `RendererBindings` / Layout 声明仍需手工同步。
- 后续仍需要 shader 反射或代码生成，把资源布局升级为单一真相源。
