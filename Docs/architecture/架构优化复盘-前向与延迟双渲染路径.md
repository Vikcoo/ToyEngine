...
- 新增 `RendererBindingSlots`，集中声明当前主路径的 binding 槽位常量。
...
- CPU 侧 binding 数字从散落常量收口到 `RendererBindingSlots`

#### 仍然保留的限制

- shader 里的 `layout(binding = N)` 与 C++ 里的 `RendererBindingSlots` 仍是两份声明，还没有上升到单一真相源。
