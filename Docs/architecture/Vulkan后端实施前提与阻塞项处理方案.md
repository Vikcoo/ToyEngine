...
本轮仍没有实现任何 `Vk*` 对象、`RHIVulkan` 模块或 Vulkan 运行时路径。Shader 反射、descriptor 生命周期、frame retirement queue 与 Validation 回归属于后续阶段，不应把“前提完成”误读为“Vulkan 后端已经完成”。
...
原生对象销毁采用后端延迟释放契约：RHI C++ 包装对象可以结束高层生命周期，但 Vulkan descriptor、buffer、image 等原生对象必须在关联 frame fence 完成后真正释放。当前 OpenGL 因立即执行可直接释放；Vulkan 阶段必须实现 frame retirement queue。
...
### 阶段 A：Vulkan Bootstrap 与清屏

- 引入 Vulkan Headers/Loader、Validation Layer 和内存分配依赖。
...
- 验收：启动、连续 resize、最小化恢复和关闭均无 Validation Error。
