...
### 阶段三补充修正：材质采样器 mipmap 与各向异性过滤（已完成，2026-06-19）

斜视角观察高频 PBR 贴图时，画面会出现噪点和随相机移动抖动。根因不是贴图没有生成 mipmap，而是材质绑定使用的 sampler object 只表达 `Linear`，会覆盖纹理对象上的 mipmap min filter；同时项目没有启用各向异性过滤。

本次补充修正：
- `RHITextureFilter` 增加 `LinearMipmapLinear`。
- `RHISamplerDesc` 增加 `enableAnisotropy` 与 `maxAnisotropy`。
- OpenGL sampler 支持 `GL_LINEAR_MIPMAP_LINEAR`，并在 `GL_EXT_texture_filter_anisotropic` 存在时启用 anisotropy。
- 默认材质采样器与 Environment 采样器使用 mipmap 线性过滤和 8x anisotropy。
- GBuffer 采样器保持 `Nearest + ClampToEdge`，避免破坏法线、深度、材质参数等结构化数据。

这个修正让斜视角纹理走样明显降低，但它不是最终的材质抗闪烁方案。后续如果 normal map 高频细节仍导致高光闪烁，还需要继续考虑 Toksvig / LEAN、roughness mip 调整或更完整的纹理导入预处理。
