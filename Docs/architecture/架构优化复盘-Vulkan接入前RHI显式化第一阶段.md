...
新增 `RHIDeviceCreateDesc`、`RHIFrameBeginInfo`、`RHIFrameContext` 和 `RHIFrameStatus`，由 `RHIDevice::BeginFrame/EndFrame` 建立当前帧、命令缓冲和呈现边界。OpenGL Device 在 `EndFrame` 调用平台呈现回调；未来 Vulkan Device 可在同一接口后执行 Acquire、Submit 与 Present。
